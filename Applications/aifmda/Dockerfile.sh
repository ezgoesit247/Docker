FROM local/u18-java8 as intermediate

RUN apt-get -qq update \
&& apt-get install -qq \
sudo \
git \
mysql-client \
&& apt-get -qq clean

ARG R=root
ARG RDIR=
ARG RDIRPATH=/root
ARG RDIR_SAFE_PATH=\\/root

ENV GIT_SSH=$RDIRPATH/bin/git-ssh
ARG GIT_CONFIG=$RDIRPATH/.gitconfig
ARG KNOWN_HOSTS=$RDIRPATH/.ssh/known_hosts
ARG SSH_PRIVATE_KEY=$RDIRPATH/.ssh/id_ed25519.ezgoesit247.github
ARG SSH_PRIVATE_KEY_STREAM

RUN mkdir $RDIRPATH/bin \
&& mkdir $RDIRPATH/.ssh

COPY assets.docker/git-ssh $GIT_SSH
COPY assets.docker/.gitconfig $GIT_CONFIG
COPY assets.docker/known_hosts $KNOWN_HOSTS
COPY assets.docker/id_ed25519.ezgoesit247.github $SSH_PRIVATE_KEY

RUN chmod 700 $RDIRPATH/.ssh \
&& chmod 755 $RDIRPATH/bin \
&& chmod 755 $GIT_SSH \
&& chmod 600 $KNOWN_HOSTS \
&& chmod 644 $GIT_CONFIG \
&& sed -i 's/\/Users\/brandonindia/'$RDIR_SAFE_PATH'/' $GIT_CONFIG \
\
#&& echo "${SSH_PRIVATE_KEY_STREAM}" > $SSH_PRIVATE_KEY \
&& chmod 600 $SSH_PRIVATE_KEY





ARG U=poweruser
ARG UDIR=/home
ARG UDIRPATH=$UDIR/$U
ARG UDIR_SAFE_PATH=\\/home\\/poweruser


RUN echo "ALL ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers \
&& useradd -ms /bin/bash -d $UDIRPATH -U $U \
&& mkdir $UDIRPATH/bin


ENV GIT_SSH=$UDIRPATH/bin/git-ssh
ARG GIT_CONFIG=$UDIRPATH/.gitconfig
ARG KNOWN_HOSTS=$UDIRPATH/.ssh/known_hosts
ARG SSH_PRIVATE_KEY=$UDIRPATH/.ssh/id_ed25519.ezgoesit247.github
ARG SSH_PRIVATE_KEY_STREAM



COPY assets.docker/git-ssh $GIT_SSH
COPY assets.docker/.gitconfig $GIT_CONFIG
COPY assets.docker/known_hosts $KNOWN_HOSTS
COPY assets.docker/id_ed25519.ezgoesit247.github $SSH_PRIVATE_KEY

RUN chmod 700 $UDIRPATH/.ssh \
&& chmod 755 $UDIRPATH/bin \
&& chmod 755 $GIT_SSH \
&& chmod 600 $KNOWN_HOSTS \
&& chmod 644 $GIT_CONFIG \
&& sed -i 's/\/Users\/brandonindia/'$UDIR_SAFE_PATH'/' $GIT_CONFIG \
\
#&& echo "${SSH_PRIVATE_KEY_STREAM}" > $SSH_PRIVATE_KEY \
&& chmod 600 $SSH_PRIVATE_KEY \
&& chown -R $U:$U $UDIR/*

VOLUME /mda
RUN git clone git@github.com:ezgoesit247/MDA /mda \
&& chown -R poweruser:poweruser /mda /mda/.git


RUN echo 'alias ls="ls -Altr --color=auto"\n\
export PS1="${debian_chroot:+($debian_chroot)}\[\033[1;34m\]\u\[\033[0m\]@\[\033[1;31m\]\h:\[\033[0;37m\]\w\[\033[0m\]\$ " \n\
'\
>> /home/poweruser/.bashrc


RUN echo 'export HISTTIMEFORMAT="%F	%T	"\n\
'\
>> /home/poweruser/.bashrc

RUN apt-get -qq clean

USER $U
WORKDIR $UDIRPATH
ENV DOCKER_ENV=aifmda
ENV DOCKER_ENV=$DOCKER_ENV


RUN echo 'if [ -d ${HOME}/_assets/bash_history/ ]; then export HISTFILE="${HOME}/_assets/bash_history/history.'$DOCKER_ENV'"; fi && green "Shared bash history at: " && echo ${HISTFILE}\n\
'\
>> /home/poweruser/.bashrc
