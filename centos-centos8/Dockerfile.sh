# docker build --rm -t local/centos-centos8 --build-arg SSH_PRIVATE_KEY_STREAM="$(cat ~/.ssh/id_ed25519.ezgoesit247.github)" .
# build --arg=SSH_PRIVATE_KEY=${GITKEYNAME} --stream SSH_PRIVATE_KEY_STREAM ~/.ssh/${GITKEYNAME}
...
&& sed -i 's/\/Users\/brandonindia/'$ROOT_SAFE_PATH'/' $GIT_CONFIG \
